...
class PointDataService:
    ...
    def _build_spatial_index(self):
        ...
        if self.all_data is not None and not self.all_data.empty:
            # 提取经纬度坐标
            coordinates = self.all_data[['lat', 'lng']].values
            self.spatial_index = cKDTree(coordinates)
    ...
    #
    #     # 假设self.data_points是包含所有数据点的列表 [(lat1, lng1), (lat2, lng2), ...]
    #     points = [(point['lat'], point['lng']) for point in self.data_points]
