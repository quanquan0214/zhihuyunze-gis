...
#
#             # Step 2: 替换RSEI、温度和降水中的NaN值为0
#             rsei_data = np.nan_to_num(rsei_data, nan=0.0)
#             temp_data = np.nan_to_num(temp_data, nan=0.0)
...
#
#                 # 将 LUCC 值为 0 的视为无效（设置为 1）
#                 lucc_raw = lucc_raw.astype(np.float64)  # 转换为浮点型以避免类型问题
...
#
#             # Step 6: 去除 X 或 y 中含 NaN/inf 的所有数据行
...
#                 # 将 LUCC 值为 0 的视为无效（设置为 1）
#                 lucc_valid = lucc_valid.astype(np.float64)  # 转换为浮点型以避免类型问题
#                 lucc_valid[lucc_valid == 0] = 1
#
#                 if lucc_encoding == 'continuous':
...
class GWRService:
    ...
    def get_gwr_input_data(self, year: int, include_lucc: bool = True, lucc_encoding: str = 'continuous') -> Tuple[
        ...
        try:
            ...
            # Step 5: 检查数据方差
            self.logger.info(f"RSEI variance: {np.var(rsei_valid):.4f}")
            self.logger.info(f"Temperature variance: {np.var(temp_valid):.4f}")
            self.logger.info(f"Rainfall variance: {np.var(rain_valid):.4f}")
            ...
            if include_lucc:
                ...
                lucc_raw = lucc_data[valid_mask].reshape(-1, 1)
                lucc_raw = lucc_raw.astype(np.float64)  # 转换为浮点型以避免类型问题
                lucc_raw[lucc_raw == 0] = 1  # 将 LUCC 值为 0 的设置为 1

                self.logger.info(f"LUCC unique values: {np.unique(lucc_raw)}")
                self.logger.info(f"LUCC variance before encoding: {np.var(lucc_raw):.4f}")

                if lucc_encoding == 'continuous':
                    max_lucc = np.nanmax(lucc_raw)
                    if max_lucc == 0:
                        self.logger.warning("LUCC 数据全为 0 或 1，方差可能为 0")
                        lucc_valid = lucc_raw  # 避免除以零
                    else:
                        lucc_valid = lucc_raw / max_lucc
                    self.logger.info(f"LUCC variance after continuous encoding: {np.var(lucc_valid):.4f}")
                elif lucc_encoding == 'dummy':
                    try:
                        if float(sklearn.__version__.split('.')[0]) >= 1 and float(
                                sklearn.__version__.split('.')[1]) >= 2:
                            ...
                            encoder = OneHotEncoder(sparse=False, handle_unknown='ignore')
                        lucc_valid = encoder.fit_transform(lucc_raw)
                        self.logger.info(f"LUCC dummy encoding shape: {lucc_valid.shape}")
                        for i in range(lucc_valid.shape[1]):
                            self.logger.info(f"LUCC category {i} variance: {np.var(lucc_valid[:, i]):.4f}")
                    except Exception as e:
                        self.logger.error(f"LUCC编码失败: {str(e)}")
                        raise
                else:
            ...
            # # Log final variances
            # self.logger.info(f"Final RSEI variance: {np.var(rsei_valid):.4f}")
            # self.logger.info(f"Final X variances: {[np.var(X[:, i]):.4f for i in range(X.shape[1])]}")
    ...
    def get_gwr_input_data_by_extent(self, year: int, extent: Tuple[float, float, float, float],
        ...
        try:
            ...
            if include_lucc:
                ...
                self.logger.info(f"LUCC unique values: {np.unique(lucc_valid)}")
                self.logger.info(f"LUCC variance before encoding: {np.var(lucc_valid):.4f}")

                if lucc_encoding == 'continuous':
                    max_lucc = np.nanmax(lucc_valid)
                    if max_lucc == 0:
                        self.logger.warning("LUCC 数据全为 0 或 1，方差可能为 0")
                        lucc_valid = lucc_valid
                    else:
                        lucc_valid = lucc_valid / max_lucc
                    self.logger.info(f"LUCC variance after continuous encoding: {np.var(lucc_valid):.4f}")
                elif lucc_encoding == 'dummy':
                    try:
                        if float(sklearn.__version__.split('.')[0]) >= 1 and float(
    ...
    def run_gwr(self, year: int, include_lucc: bool = True, bw: Optional[float] = None,
        ...
        try:
            ...
            # 避免除以零，设置小标准差
            min_std = 1e-8
            X_std = np.where(X_std == 0, min_std, X_std)
            y_std = min_std if y_std == 0 else y_std

            # self.logger.info(f"y_std: {y_std:.4f}")
            # self.logger.info(f"X_std: {[X_std[i]:.4f for i in range(X.shape[1])]}")

            X_stdized = (X - X_mean) / X_std
            y_stdized = (y - y_mean) / y_std

            # Step 4: 再次检查 NaN/inf
            if np.any(np.isnan(X_stdized)) or np.any(np.isinf(X_stdized)) or \
                    np.any(np.isnan(y_stdized)) or np.any(np.isinf(y_stdized)):
                raise ValueError("标准化后的数据中仍包含 NaN 或 inf 值")
