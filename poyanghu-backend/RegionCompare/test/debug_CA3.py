...
# 定义四个区域的geojson文件路径（新增两个城市）
jiujiang_geojson = "D:/Google/GLC_FCS30/九江市_市.geojson"
nanchang_geojson = "D:/Google/GLC_FCS30/南昌市_市.geojson"
fuzhou_geojson = "D:/Google/GLC_FCS30/抚州市_市.geojson"  # 新增
jingdezhen_geojson = "D:/Google/GLC_FCS30/景德镇市_市.geojson"  # 新增
...
# 新增的两个城市
with open(fuzhou_geojson, 'r', encoding='utf-8') as f:  # 新增
    fuzhou_data = {'data': f.read(), 'name': '抚州市'}  # 新增
    fuzhou_id = analyzer.add_analysis_region(fuzhou_data, region_type='vector')  # 新增

with open(jingdezhen_geojson, 'r', encoding='utf-8') as f:  # 新增
    jingdezhen_data = {'data': f.read(), 'name': '景德镇市'}  # 新增
    jingdezhen_id = analyzer.add_analysis_region(jingdezhen_data, region_type='vector')  # 新增

# 定义分析年份范围 (2000-2022)
analysis_years = list(range(2000, 2022))

# 执行并行对比分析（修改region_ids列表）
results = analyzer.parallel_comparison_analysis(
    region_ids=[jiujiang_id, nanchang_id, fuzhou_id, jingdezhen_id],  # 新增两个ID
